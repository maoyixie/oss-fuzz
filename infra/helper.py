...
class Project:
    ...
    def __init__(
        ...
        if self.is_external:
            ...
            self.build_integration_path = os.path.join(
                self.path, build_integration_path
            )
        else:
    ...
    def language(self):
        ...
        project_yaml_path = os.path.join(self.build_integration_path, "project.yaml")
...
def get_parser():  # pylint: disable=too-many-statements,too-many-locals
    ...
    build_image_parser = subparsers.add_parser("build_image", help="Build an image.")
    ...
    build_fuzzers_parser.add_argument(
        "--mount_path",
        dest="mount_path",
        help="path to mount local source in " "(defaults to WORKDIR)",
    )
    build_fuzzers_parser.add_argument(
        "--clean", dest="clean", action="store_true", help="clean existing artifacts."
    )
    build_fuzzers_parser.add_argument(
        "--no-clean",
        dest="clean",
        action="store_false",
        help="do not clean existing artifacts " "(default).",
    )
    build_fuzzers_parser.set_defaults(clean=False)

    fuzzbench_build_fuzzers_parser = subparsers.add_parser("fuzzbench_build_fuzzers")
    ...
    check_build_parser.add_argument("fuzzer_name", help="name of the fuzzer", nargs="?")
    ...
    fuzzbench_run_fuzzer_parser.add_argument("fuzzer_name", help="name of the fuzzer")
    fuzzbench_run_fuzzer_parser.add_argument(
        "fuzzer_args", help="arguments to pass to the fuzzer", nargs="*"
    )
    ...
    fuzzbench_measure_parser.add_argument("engine_name", help="name of the fuzzer")
    fuzzbench_measure_parser.add_argument("fuzz_target_name", help="name of the fuzzer")
    ...
    coverage_parser.add_argument(
        "--no-serve", action="store_true", help="do not serve a local HTTP server."
    )
    coverage_parser.add_argument(
        "--port",
        default="8008",
        help="specify port for" " a local HTTP server rendering coverage report",
    )
    coverage_parser.add_argument(
        "--fuzz-target",
    ...
    coverage_parser.add_argument(
        "--public",
        action="store_true",
        help="if set, will download public " "corpus using wget",
    )
    coverage_parser.add_argument(
        "project", help="name of the project or path (external)"
    )
    coverage_parser.add_argument(
        "extra_args",
        help="additional arguments to " "pass to llvm-cov utility.",
        nargs="*",
    )
    ...
    introspector_parser.add_argument(
        "--seconds", help="number of seconds to run fuzzers", default=10
    )
    introspector_parser.add_argument(
        "source_path", help="path of local source", nargs="?"
    )
    ...
    download_corpora_parser.add_argument(
        "--fuzz-target", nargs="+", help="specify name of a fuzz target"
    )
    download_corpora_parser.add_argument(
        "--public",
        action="store_true",
    ...
    reproduce_parser = subparsers.add_parser("reproduce", help="Reproduce a crash.")
    ...
    shell_parser.add_argument("project", help="name of the project or path (external)")
    shell_parser.add_argument("source_path", help="path of local source", nargs="?")
    ...
    run_clusterfuzzlite_parser.add_argument("--branch", default="master", required=True)
...
def _check_fuzzer_exists(project, fuzzer_name, architecture="x86_64"):
    ...
    try:
        ...
        logger.error(
            "%s does not seem to exist. Please run build_fuzzers first.", fuzzer_name
        )
        return False
...
def _add_architecture_args(parser, choices=None):
    ...
    parser.add_argument(
        "--architecture", default=constants.DEFAULT_ARCHITECTURE, choices=choices
    )
...
def _add_engine_args(parser, choices=None):
    ...
    parser.add_argument("--engine", default=constants.DEFAULT_ENGINE, choices=choices)
...
def _add_sanitizer_args(parser, choices=None):
    ...
    parser.add_argument(
        "--sanitizer", default=None, choices=choices, help='the default is "address"'
    )
...
def prepare_aarch64_emulation():
    ...
    subprocess.check_call(["docker", "buildx", "create", "--name", ARM_BUILDER_NAME])
...
def build_image(args):
    ...
    if build_image_impl(
        args.project, cache=args.cache, pull=pull, architecture=args.architecture
    ):
        return True
...
def build_fuzzers_impl(  # pylint: disable=too-many-arguments,too-many-locals,too-many-branches
    ...
    """Builds fuzzers."""
    if build_project_image and not build_image_impl(project, architecture=architecture):
        return False
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        assert docker_build(
            [
                ...
                "--file",
                os.path.join(
                    fuzzbench_path, "fuzzers", args.engine, "builder.Dockerfile"
                ),
...
def _get_latest_corpus(project, fuzz_target, base_corpus_dir):
    ...
    corpus_backup_url = CORPUS_BACKUP_URL_FORMAT.format(
        project_name=project.name, fuzz_target=fuzz_target
    )
...
def _get_latest_public_corpus(args, fuzzer):
    ...
    download_url = HTTPS_CORPUS_BACKUP_URL_FORMAT.format(
        project_name=args.project.name, fuzz_target=project_qualified_fuzz_target_name
    )
...
def download_corpora(args):
    ...
    if args.public:
        ...
        try:
            ...
            logger.error(
                "gsutil not found. Please install it from "
    ...
    if not fuzz_targets:
        logger.error(
            ...
            "can automatically identify targets.",
            args.project.name,
        )
        return False
    ...
    def _download_for_single_target(fuzz_target):
        try:
            ...
            logger.error("Corpus download for %s failed: %s.", fuzz_target, str(error))
            return False

    logger.info(
        "Downloading corpora for %s project to %s.", args.project.name, corpus_dir
    )
...
def coverage(args):
    ...
    if (
        not args.no_corpus_download
        and not args.corpus_dir
        and not args.project.is_external
    ):
...
def introspector(args):
    ...
    for target_cov_dir in os.listdir(src_target_cov_report):
        ...
        shutil.copytree(
            os.path.join(src_target_cov_report, target_cov_dir), dst_target_cov_report
        )
        ...
        run_args.extend(
            [
                ...
                "{corpus_dir}:/tmp/{fuzzer}_corpus".format(
                    corpus_dir=corpus_dir, fuzzer=args.fuzzer_name
                ),
            ]
        )
...
def fuzzbench_measure(args):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        run_args = [
            ...
            "EXPERIMENT_TYPE=bug",
            "-e",
            f"FUZZ_TARGET={args.fuzz_target_name}",
            "-e",
            f"FUZZER={args.engine_name}",
            "-e",
            f"BENCHMARK={args.project.name}",
            f"gcr.io/oss-fuzz/{args.project.name}",
            "fuzzbench_measure",
